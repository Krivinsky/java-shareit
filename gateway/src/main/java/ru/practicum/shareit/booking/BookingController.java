...
public class BookingController {
	...
	@PostMapping
	public ResponseEntity<Object> creatBooking(@RequestHeader("X-Sharer-User-Id") long userId,
											   @RequestBody @Valid BookItemRequestDto requestDto) {
		log.info("Creating booking {}, userId={}", requestDto, userId);
		return bookingClient.creatItem(userId, requestDto);
	}
	...
	public ResponseEntity<Object> getOwnerItemsAll(@RequestHeader ("X-Sharer-User-Id") long userId,
		...
		Status state = Status.from(stateParam)
				.orElseThrow(() -> new IllegalArgumentException("Unknown state: " + stateParam));
		return bookingClient.getOwnerItemsAll(userId, from, size, state);
	}
